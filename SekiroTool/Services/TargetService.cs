...
public class TargetService(IMemoryService memoryService, HookManager hookManager, IReminderService reminderService) : ITargetService
{
    ...
    public void ToggleTargetHook(bool isEnabled)
    {
        ...
        if (isEnabled)
        {
            ...
            AsmHelper.WriteRelativeOffsets(bytes, [
                (code, ptrLoc, 7, 0x0 + 3),
                (code + 0xE, hookLoc + 0x7, 5, 0xE + 1)
            ]);

            memoryService.WriteBytes(code, bytes);
            hookManager.InstallHook(code, hookLoc, [0x48, 0x8B, 0x80, 0xF8, 0x1F, 0x00, 0x00]);
        }
        else
        {
    ...
    public nint GetTargetChrIns() =>
        memoryService.Read<nint>(CodeCaveOffsets.Base + CodeCaveOffsets.LockedTarget);
    ...
    public float GetCurrentPoise() =>
        memoryService.Read<float>(GetChrSuperArmorPtr() + (int)ChrIns.ChrSuperArmorOffsets.Poise);

    public float GetMaxPoise() =>
        memoryService.Read<float>(GetChrSuperArmorPtr() + (int)ChrIns.ChrSuperArmorOffsets.MaxPoise);

    public float GetPoiseTimer() =>
        memoryService.Read<float>(GetChrSuperArmorPtr() + (int)ChrIns.ChrSuperArmorOffsets.PoiseTimer);

    public int GetCurrentPoison() =>
        memoryService.Read<int>(GetChrResistPtr() + (int)ChrIns.ChrResistOffsets.PoisonCurrent);

    public int GetMaxPoison() =>
        memoryService.Read<int>(GetChrResistPtr() + (int)ChrIns.ChrResistOffsets.PoisonMax);

    public int GetCurrentBurn() =>
        memoryService.Read<int>(GetChrResistPtr() + (int)ChrIns.ChrResistOffsets.BurnCurrent);

    public int GetMaxBurn() =>
        memoryService.Read<int>(GetChrResistPtr() + (int)ChrIns.ChrResistOffsets.BurnMax);

    public int GetCurrentShock() =>
        memoryService.Read<int>(GetChrResistPtr() + (int)ChrIns.ChrResistOffsets.ShockCurrent);

    public int GetMaxShock() =>
        memoryService.Read<int>(GetChrResistPtr() + (int)ChrIns.ChrResistOffsets.ShockMax);
    ...
    public float[] GetPosition()
    {
        ...
        position[0] = memoryService.Read<float>(posPtr);
        position[1] = memoryService.Read<float>(posPtr + 0x4);
        position[2] = memoryService.Read<float>(posPtr + 0x8);
    ...
    public void ToggleFreezePosture(bool isEnabled)
    {
        ...
        if (isEnabled)
        {
            ...
            var bytes = AsmLoader.GetAsmBytes(AsmScript.FreezeTargetPosture);

            AsmHelper.WriteRelativeOffsets(bytes, [
                (code + 0x1, lockedTargetPtr, 7, 0x1 + 3),
                (code + 0x14, endOfHookedFunc, 6, 0x14 + 2),
                (code + 0x1F, hookLoc + 0x5, 5, 0x1F + 1)
            ]);

            memoryService.WriteBytes(code, bytes);
            hookManager.InstallHook(code, hookLoc, [0x48, 0x89, 0x5C, 0x24, 0x30]);
        }
        else
        {
            hookManager.UninstallHook(code);
    ...
    {
        var bitFlagStart = memoryService.Read<nint>(CodeCaveOffsets.Base + CodeCaveOffsets.LockedTarget)
    ...
    {
        var targetingSystemPtr = memoryService.Read<nint>(GetAiThinkPtr() + (int)ChrIns.AiThinkOffsets.TargetingSystem);
        memoryService.SetBitValue(targetingSystemPtr + ChrIns.TargetView.Offset, ChrIns.TargetView.Bit,
            isEnabled);
    }
    ...
    {
        var targetingSystemPtr = memoryService.Read<nint>(GetAiThinkPtr() + (int)ChrIns.AiThinkOffsets.TargetingSystem);
        return memoryService.IsBitSet(targetingSystemPtr + ChrIns.TargetView.Offset, ChrIns.TargetView.Bit);
    }

    public int GetLastAct() =>
        memoryService.Read<byte>(GetAiThinkPtr() + (int)ChrIns.AiThinkOffsets.LastAct);

    public int GetLastKengekiAct() =>
        memoryService.Read<byte>(GetAiThinkPtr() + (int)ChrIns.AiThinkOffsets.LastKengekiAct);

    public void ForceAct(int act) =>
        memoryService.Write(GetAiThinkPtr() + (int)ChrIns.AiThinkOffsets.ForceAct, (byte)act);

    public void ForceKengekiAct(int act) =>
        memoryService.Write(GetAiThinkPtr() + (int)ChrIns.AiThinkOffsets.ForceKengekiAct, (byte)act);

    public bool IsTargetRepeating() =>
        memoryService.Read<byte>(GetAiThinkPtr() + (int)ChrIns.AiThinkOffsets.ForceAct) != 0;

    public bool IsTargetRepeatingKengeki() =>
        memoryService.Read<byte>(GetAiThinkPtr() + (int)ChrIns.AiThinkOffsets.ForceKengekiAct) != 0;
    ...
    public void ToggleTargetRepeatAct(bool isEnabled)
    {
        ...
        byte value = isEnabled
            ? memoryService.Read<byte>(aiThinkPtr + (int)ChrIns.AiThinkOffsets.LastAct)
            : (byte)0;

        memoryService.Write(forceActPtr, value);
    }
    ...
    public void ToggleTargetRepeatKengekiAct(bool isEnabled)
    {
        ...
        byte value = isEnabled
            ? memoryService.Read<byte>(aiThinkPtr + (int)ChrIns.AiThinkOffsets.LastKengekiAct)
            : (byte)0;

        memoryService.Write(forceActPtr, value);
    }

    public uint GetTargetHandle() => memoryService.Read<uint>(GetTargetChrIns() + ChrIns.Handle);

    public uint GetCharacterId() => memoryService.Read<uint>(GetTargetChrIns() + ChrIns.CharacterId);

    public int GetEntityId() => memoryService.Read<int>(GetTargetChrIns() + ChrIns.EntityId);
